...
class TypeFrame(wx.Frame):
        ...
        try:
            for i in xrange(self.grid.GetNumberRows()):
                if tb.GetValue(i, 1) != '' and i == self.grid.GetGridCursorRow():
                    cur.callproc('"AddType"', [tb.GetValue(i, 1), tb.GetValue(i, 2), tb.GetValue(i, 3), tb.GetValue(i, 4)])
    ...
    def on_update(self, event):
        ...
        try:
            for i in xrange(self.grid.GetNumberRows()):
                if tb.GetValue(i, 1) != '' and i == self.grid.GetGridCursorRow():
                    if (wx.MessageBox('Realy update type?','Update Type',wx.YES_NO|wx.ICON_QUESTION)) == 2:
                        cur.callproc('"updateType"', [tb.GetValue(i, 0), tb.GetValue(i, 1), tb.GetValue(i, 2), tb.GetValue(i, 3), tb.GetValue(i, 4)])
        except psycopg2.DatabaseError as e: wx.MessageBox(str(e),'Error',wx.OK|wx.ICON_ERROR)
        conn.commit()
        cur.close()
        conn.close()
        ...
        try:
            for i in xrange(self.grid.GetNumberRows()):
                if i == self.grid.GetGridCursorRow():
                    if (wx.MessageBox('Realy?','Delete Type',wx.YES_NO|wx.ICON_QUESTION)) == 2:
                        cur.callproc('"deleteType"', [tb.GetValue(i, 0)])
        except psycopg2.DatabaseError as e: wx.MessageBox(str(e),'Error',wx.OK|wx.ICON_ERROR)
        conn.commit()
        cur.close()
        conn.close()
