...
class PersonFrame(wx.Frame):
    def __init__(self, parent, id):
        ...
        wx.Frame.__init__(self, parent, id, 'Working with Personal', \
        ...
        self.grid.SetColLabelValue(3, 'Property')
        self.grid.SetColSize(3, 120)
        self.grid.SetColLabelValue(4, 'Contact')
        self.grid.SetColSize(4,120)
        self.grid.SetColLabelValue(5, 'Phone')
        self.grid.SetColSize(5,60)
        ...
        try:
            for i in xrange(self.grid.GetNumberRows()):
                if tb.GetValue(i, 1) != '' and i == self.grid.GetGridCursorRow():
                    cur.callproc('"AddPersonal"', [tb.GetValue(i, 1), tb.GetValue(i, 2), tb.GetValue(i, 3), tb.GetValue(i, 4), tb.GetValue(i, 5)])
    ...
    def on_update(self, event):
        ...
        try:
            for i in xrange(self.grid.GetNumberRows()):
                if tb.GetValue(i, 1) != '' and i == self.grid.GetGridCursorRow():
                    if (wx.MessageBox('Realy update person named '+tb.GetValue(i, 1)+' ?','Update Person',wx.YES_NO|wx.ICON_QUESTION)) == 2:
                        cur.callproc('"updatePerson"', [tb.GetValue(i, 0), tb.GetValue(i, 1), tb.GetValue(i, 2), tb.GetValue(i, 3), tb.GetValue(i, 4), tb.GetValue(i, 5)])
        except psycopg2.DatabaseError as e: wx.MessageBox(str(e),'Error',wx.OK|wx.ICON_ERROR)
        conn.commit()
        cur.close()
        conn.close()
        ...
        try:
            for i in xrange(self.grid.GetNumberRows()):
                if i == self.grid.GetGridCursorRow():
                    if (wx.MessageBox('Realy?','Delete Person',wx.YES_NO|wx.ICON_QUESTION)) == 2:
                        cur.callproc('"deletePersonal"', [tb.GetValue(i, 0)])
        except psycopg2.DatabaseError as e: wx.MessageBox(str(e),'Error',wx.OK|wx.ICON_ERROR)
        conn.commit()
        cur.close()
        conn.close()
